...
sirf::shared_ptr<aGadget> 
GadgetChain::gadget_sptr(std::string id)
{
#ifdef _MSC_VER
	std::list<sirf::shared_ptr<GadgetHandle> >::iterator gh;
#else
	typename std::list<sirf::shared_ptr<GadgetHandle> >::iterator gh;
#endif
	for (gh = gadgets_.begin(); gh != gadgets_.end(); gh++) {
		if (boost::iequals(gh->get()->id(), id))
			return gh->get()->gadget_sptr();
	}
	return sirf::shared_ptr<aGadget>();
}
...
#ifdef _MSC_VER
	std::list<sirf::shared_ptr<GadgetHandle> >::const_iterator gh;
#else
	typename std::list<sirf::shared_ptr<GadgetHandle> >::const_iterator gh;
#endif
