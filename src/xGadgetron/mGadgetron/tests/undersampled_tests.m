function undersampled_tests(engine)
...
ntest = ntest + 1;
failed = failed + test_failed(ntest, 147.9937, acq_data_norm, 0, eps);
...
ntest = ntest + 1;
failed = failed + test_failed(ntest, 142.3433, processed_data_norm, 0, eps);
...
ntest = ntest + 1;
failed = failed + test_failed(ntest, 996.5223, complex_images_norm, 0, eps);
...
fprintf('---\n reconstructed images forward projection norm %e\n', ...
      fwd_acqs_norm)
ntest = ntest + 1;
failed = failed + test_failed(ntest, 971.9004, fwd_acqs_norm, 0, eps);

acqs_diff = fwd_acqs - processed_data;
rr = acqs_diff.norm()/fwd_acqs_norm;
fprintf('---\n reconstruction residual norm (rel): %e\n', rr)
ntest = ntest + 1;
failed = failed + test_failed(ntest, 0.8598439, rr, eps, 0);
...
fprintf(...
    ...
    rd)
ntest = ntest + 1;
failed = failed + test_failed(ntest, 0.8665925, rd, eps, 0);
...
fprintf('= (B x, y) = (%e, %e)\n', real(Bxy), imag(Bxy))
ntest = ntest + 1;
failed = failed + test_failed(ntest, real(xFy), real(Bxy), 0, eps);
