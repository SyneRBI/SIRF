...
FUNCTION(get_list_of_wrapped_methods methods_file NR_VERSION result_variable)
  read_methods(methods_file, methods_list, ${NR_VERSION}, NR_VERSION_to_use)
  FOREACH(method_line ${methods_list})
    # make into list
    IF("${method_line}" MATCHES "${NR_VERSION_to_use}")
      ...
      STRING(STRIP "${type1}" type1)
      STRING(STRIP "${type2}" type2) 
      # Indent for all but first line
      IF(output)
        STRING(APPEND output "                 ")
      ENDIF()
      STRING(APPEND output "\"${command}(")
      IF(NOT "${type2}" MATCHES "-")
        STRING(APPEND output "${type1},${type2}")
      ELSEIF(NOT "${type1}" MATCHES "-")
        MESSAGE(WARNING "type1 = \"${type1}\"")
        #string(REGEX REPLACE "\n$" "" <varname> "${<varname>}"))
        STRING(APPEND output "${type1}")
      ENDIF()
      STRING(APPEND output ")\\" "n\"\n")
    ENDIF()
  ENDFOREACH()
