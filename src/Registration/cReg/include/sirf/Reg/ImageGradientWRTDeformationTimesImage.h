...
public:
    ...
    virtual void forward(std::shared_ptr<ImageData> out_sptr, const std::shared_ptr<const NiftiImageData3DDeformation<dataType> > deformation_sptr, const std::shared_ptr<const ImageData> in_sptr);

    /// Forward (resample image)
    virtual std::shared_ptr<ImageData> forward(const std::shared_ptr<const NiftiImageData3DDeformation<dataType> > deformation_sptr, const std::shared_ptr<const ImageData> in_sptr);

    /// Backward in place (get image gradient wrt transformation)
    virtual void backward(
        std::shared_ptr<NiftiImageData3DDeformation<dataType> > &output_transformation_sptr, 
    ...
    virtual std::shared_ptr<NiftiImageData3DDeformation<dataType> > backward(
        const std::shared_ptr<const NiftiImageData3DDeformation<dataType> > &deformation_sptr, 
        const std::shared_ptr<const ImageData> &image_for_gradient_sptr,
        const std::shared_ptr<const ImageData> &image_to_multiply_sptr);
...
private:
    ...
    /// Deformation
    std::shared_ptr<const NiftiImageData3DDeformation<dataType> > _deformation_sptr;
};
}
