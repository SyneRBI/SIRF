...
bool Transformation::CheckDVFImageUpdateRequired(const nifti_image * const targetImageIn ) const
{
  ...
  if ( targetImageIn->sform_code > 0 )
  {
    ...
    if ( targetImageIn->sto_ijk.m[0][0] != this->deformationVectorFieldImage->sto_ijk.m[0][0] ) return true;
    if ( targetImageIn->sto_ijk.m[0][1] != this->deformationVectorFieldImage->sto_ijk.m[0][1] ) return true;
    if ( targetImageIn->sto_ijk.m[0][2] != this->deformationVectorFieldImage->sto_ijk.m[0][2] ) return true;
    if ( targetImageIn->sto_ijk.m[0][3] != this->deformationVectorFieldImage->sto_ijk.m[0][3] ) return true;

    if ( targetImageIn->sto_ijk.m[1][0] != this->deformationVectorFieldImage->sto_ijk.m[1][0] ) return true;
    if ( targetImageIn->sto_ijk.m[1][1] != this->deformationVectorFieldImage->sto_ijk.m[1][1] ) return true;
  ...
  else
  {
    ...
    if ( targetImageIn->qto_ijk.m[0][0] != this->deformationVectorFieldImage->qto_ijk.m[0][0] ) return true;
    if ( targetImageIn->qto_ijk.m[0][1] != this->deformationVectorFieldImage->qto_ijk.m[0][1] ) return true;
    if ( targetImageIn->qto_ijk.m[0][2] != this->deformationVectorFieldImage->qto_ijk.m[0][2] ) return true;
    if ( targetImageIn->qto_ijk.m[0][3] != this->deformationVectorFieldImage->qto_ijk.m[0][3] ) return true;

    if ( targetImageIn->qto_ijk.m[1][0] != this->deformationVectorFieldImage->qto_ijk.m[1][0] ) return true;
    if ( targetImageIn->qto_ijk.m[1][1] != this->deformationVectorFieldImage->qto_ijk.m[1][1] ) return true;
