...
int test2()
{
	...
	int status = 1;
	try {
		std::string data_path = sirf::examples_data_path("PET");
		...
		for (;;) {
			...
			filename = append_path(data_path, "mMR", "mMR_template_span11_small.hs", NULL);
			...
			filename = append_path(data_path, "mMR", "mu_map.hv", NULL);
