...
(sirf::shared_ptr<PETAcquisitionData> sptr_data)
{
	////sirf::shared_ptr<ProjData> sptr(new ProjDataInMemory(*sptr_data));
	//sirf::shared_ptr<ProjData> sptr(new ProjDataInMemory(*sptr_data->data()));
...
PETAcquisitionModel::forward(const Image3DF& image)
{
	...
	sirf::shared_ptr<ProjData> sptr_fd = sptr_ad->data();
...
sirf::shared_ptr<Image3DF> 
PETAcquisitionModel::backward(ProjData& ad)
{
	sirf::shared_ptr<Image3DF> sptr_im(sptr_image_template_->clone());
	sptr_im->fill(0.0);
