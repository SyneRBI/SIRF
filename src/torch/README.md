...
For example, a sinogram in SIRF has shape `[tof bins, sinograms, views, tang pos]`. For a single non-tof sinogram this is `[1, 1, views, tang pos]`. The expected torch tensor shape for this wrapper is `[batch, [channel,] 1, 1, views, tang pos]`. On the other hand a 2D convolution requires `[batch, [channel,] height, width]`.
