...
def main():
    ...
    spdhg = SPDHG(f=f, g=G, operator=K, sigma=None, tau=None,
                max_iteration=1000,
    ...
    for i in range(1, num_iters+1, save_interval):
        spdhg.run(save_interval, verbose=True, very_verbose=True)
        out = spdhg.get_output()    
        if not nifti:
